...
export class TransferApiDataSource implements TransferDataSource {
  async createTransfer(request: CreateTransferRequest): Promise<BaseValueResponse<Transfer>> {
    const response = await axiosInstance.post<BaseValueResponse<Transfer>>(BASE_URL, request);
    return response.data;
  }

  async getMyTransfers(): Promise<BaseListResponse<Transfer>> {
    const response = await axiosInstance.get<BaseListResponse<Transfer>>(`${BASE_URL}/my`);
    return response.data;
  }

  async getMyOutTransfer(): Promise<BaseListResponse<Transfer>> {
    const response = await axiosInstance.get<BaseListResponse<Transfer>>(`${BASE_URL}/my/outgoing`);
    return response.data;
  }

  async getMyInTransfer(): Promise<BaseListResponse<Transfer>> {
    const response = await axiosInstance.get<BaseListResponse<Transfer>>(`${BASE_URL}/my/incoming`);
    return response.data;
  }

  async acceptTransfer(request: ProcessTransferRequest): Promise<BaseValueResponse<Transfer>> {
    const response = await axiosInstance.post<BaseValueResponse<Transfer>>(`${BASE_URL}/accept`, request);
    return response.data;
  }

  async rejectTransfer(request: ProcessTransferRequest): Promise<BaseValueResponse<Transfer>> {
    const response = await axiosInstance.post<BaseValueResponse<Transfer>>(`${BASE_URL}/reject`, request);
    return response.data;
  }

  async getTransfer(transferID: string): Promise<BaseValueResponse<Transfer>> {
    const response = await axiosInstance.get<BaseValueResponse<Transfer>>(`${BASE_URL}/${transferID}`);
    return response.data;
  }
} 
